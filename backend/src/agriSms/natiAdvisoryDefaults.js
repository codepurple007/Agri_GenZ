/**
 * Default Meher multilingual advisory — verbatim strings from repo `nati.MD`
 * (sections 🌱 Fertilizer through 💰 Market Prices, baseline block ~lines 126–201).
 *
 * Ethiopian language keys MUST match SRS / API (`Amharic`, `Oromo`, `English`), not headings.
 */

/** Payload merged into `agriSmsStore.currentAdvisory` (no `id` — caller sets UUID). */
export function getNatiMeherAdvisorySeed() {
  const ts = () => new Date().toISOString();
  const b = natiMeherBundles;
  return {
    season: "Meher 2026",
